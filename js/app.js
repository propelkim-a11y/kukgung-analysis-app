...
document.addEventListener('DOMContentLoaded', () => {
  ...
  nodes.sceneCapture = document.getElementById('scene-capture');
  nodes.btnQuickStart = document.getElementById('btn-quick-start');
  nodes.statusText = document.getElementById('status-text');
  nodes.webcam = document.getElementById('webcam');
  
  // 2. 인트로 -> 본 화면 전환 및 비동기 하드웨어 활성화 로직
  if (nodes.btnQuickStart) {
    nodes.btnQuickStart.addEventListener('click', async () => {
      console.log('[시스템] 분석 시작 버튼 클릭됨');
      
      // A. 장면 전환을 먼저 수행 (시각적 피드백 최우선)
      if (nodes.sceneIntro && nodes.sceneCapture) {
        nodes.sceneIntro.style.display = 'none';
        nodes.sceneCapture.style.display = 'block';
        nodes.sceneIntro.classList.remove('active');
        nodes.sceneCapture.classList.add('active');
        KUKGUNG_SYSTEM.state.currentScene = 'capture';
        console.log("-> 화면 뷰 전환 성공");
      }
      
      // B. 카메라 및 센서 초기화 (비동기로 안전하게 진행)
      try {
        if (nodes.statusText) {
          nodes.statusText.innerText = "카메라 및 센서 연결 중...";
        }
        await initSystemHardware();
      } catch (err) {
        console.error('[오류] 하드웨어 초기화 실패:', err);
        if (nodes.statusText) {
          nodes.statusText.innerText = "연결 실패 (새로고침 필요)";
        }
...
/**
 * 하드웨어(카메라/센서) 초기화 및 스트림 바인딩 비동기 함수 (v22.1 이식)
 */
async function initSystemHardware() {
  const nodes = window.bowAppNodes;
  
  // 1단계: 카메라 미디어 스트림 요청 및 최적화 (60 FPS 대응)
  try {
    const constraints = {
      video: {
        facingMode: 'environment', // 후면 분석용 카메라 지정
        width: { ideal: 1280 },
        height: { ideal: 720 },
        frameRate: { ideal: 60 }
      },
    ...
    if (nodes.webcam) {
      nodes.webcam.srcObject = stream;
      nodes.webcam.onloadedmetadata = () => {
        nodes.webcam.play();
    ...
    if (nodes.statusText) {
      nodes.statusText.innerText = "시스템 정상 (60 FPS)";
    }
  } catch (e) {
    console.warn('카메라 연결 실패:', e);
    alert("정밀 자세 측정을 위해 후면 카메라 권한 승인이 필수적입니다.");
    throw e;
  }

  // 2단계: 모바일 자이로스코프 센서 레거시 연동 핸들러 가동
  initOrientationSensor();

  // 3단계: 외부 통합 센서 모듈 가동 (v22.1 호환용)
  if (window.bowGyroSensor) {
    await window.bowGyroSensor.start();
  }

  // 4단계: 실시간 캡처 인터페이스 이벤트 바인딩
...
function handleOrientationMetrics(event) {
  ...
  // 디바이스 종단 배치 각도에 기반한 수평 안정도 판별식 계산 (isLevel)
...
function initCaptureInterface() {
  ...
  if (btnCapture) {
    ...
    btnCapture.onclick = () => {
      ...
      executeCoreKukgungAnalysis(); // 5번 파트 코어 분석 알고리즘 실행
    };
  }
}
...
function executeCoreKukgungAnalysis() {
  ...
  // 6번 파트 비전 그래픽 오버레이 가시화 드로잉 모듈 킥오프
...
/**
 * 사수의 실시간 자세 데이터 프레임 위에 그래픽 가이드라인 표출
 */
function renderAnalysisGraphics(ctx, w, h, metrics) {
  ...
  // 3. 렌더된 프레임 상단 텍스트 HUD 스코어보드 오버레이 출력
