...
/**
 * 범용 웹앱 실시간 조회수 카운터 로직 (propelkim-a11y 국궁 앱 전용 락온 버전)
 */
window.addEventListener('DOMContentLoaded', () => {
  // 💡 배포 주소를 기반으로 저장소 경로 자동 고정 완료
  const repoOwner = "propelkim-a11y"; 
  const repoName = "kukgung-analysis-app";
  
  // 프로젝트 고유 키값 생성 (로컬스토리지 및 세션 동기화 충돌 차단막)
  const storageKeyViews = `app_views_${repoName}`;
  const storageKeySession = `app_session_${repoName}`;
  
  // 1. 브라우저 로컬 세션 스레드 기반 방문자 가산 처리
  let currentViews = parseInt(localStorage.getItem(storageKeyViews) || '0');
  
  if (!sessionStorage.getItem(storageKeySession)) {
    currentViews += 1;
    localStorage.setItem(storageKeyViews, currentViews);
    sessionStorage.setItem(storageKeySession, 'true');
  }
  
  // 2. DOM 노드 검증 및 깃허브 메타 인프라 주입
  const viewElement = document.getElementById('global-view-count');
  
  if (viewElement) {
    // 깃허브 공식 엔드포인트 REST 커널 파이프라인 개통
    fetch(`https://github.com{repoOwner}/${repoName}`)
      .then(res => {
        if (!res.ok) throw new Error('API 호출 한계 또는 저장소 부재');
        return res.json();
      })
      .then(data => {
        // 애플 미니멀 테마 맞춤형 정밀 스케일업 가중치 공식
        const starBonus = (data.stargazers_count || 0) * 15;   // 스타 가중치
        const forkBonus = (data.forks_count || 0) * 25;       // 포크 가중치
        const issueBonus = (data.open_issues_count || 0) * 5; // 피드백 활성 가중치
        
        // 초기 렌더링용 시작 보정치 (앱 론칭 초기 가독성 확보용)
        const baseStartingValue = 80; 
        
        // 수식 최종 합산 연산
        const totalViews = baseStartingValue + starBonus + forkBonus + issueBonus + currentViews; 
        
        // 최종 텍스트 적용 및 천 단위 포맷 바인딩
        viewElement.innerText = `Views: ${totalViews.toLocaleString()}`;
      })
      .catch((err) => {
        console.warn('[Counter Engine] GitHub Core Fallback Activated:', err.message);
        // 네트워크 오프라인 상태 혹은 API 한계 초과 시 안전 보호용 폴백 데이터 출력
        const fallbackValue = 80 + currentViews;
        viewElement.innerText = `Views: ${fallbackValue.toLocaleString()}`;
        viewElement.style.color = '#86868b'; // 오프라인 모드 진입 시 시스템 그레이로 틴트 다운
      });
  }
});
